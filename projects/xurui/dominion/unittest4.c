...
int main()
{
	...
	retVal = playCouncilRoomRefactor(&testG, 0);
	...
	assert1(G.handCount[thisPlayer]+4-1==testG.handCount[thisPlayer]);
	
	printf("deckCount: before = %d, after = %d\n", G.deckCount[thisPlayer], testG.deckCount[thisPlayer]);
	assert1(G.deckCount[thisPlayer]-4==testG.deckCount[thisPlayer]);
	
	printf("numBuys: before = %d, after = %d\n", G.numBuys, testG.numBuys);
	assert1(G.numBuys+1==testG.numBuys);
